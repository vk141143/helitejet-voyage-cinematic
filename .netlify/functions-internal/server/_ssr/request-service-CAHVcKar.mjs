...
//#region node_modules/.nitro/vite/services/ssr/assets/request-service-CAHVcKar.js
...
async function listStaffRequests() {
	...
	const { data: entitlements, error: entitlementError } = customerIds.length ? await supabase.from("customer_entitlements").select("*").in("customer_id", customerIds) : {
		data: [],
		error: null
	};
	...
	return (data ?? []).map((request) => ({
		...request,
		customer_entitlement: entitlementByCustomer.get(request.customer_id) ?? null
	}));
}
//#endregion
export { listCustomerRequests as n, listStaffRequests as r, createRequest as t };
